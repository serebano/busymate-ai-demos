...
## Configuration
This demo's own MCP server is configured at `https://discord.demo.busymate.ai/mcp` (JSON-RPC 2.0, transport: streamable-http, no auth to connect). What this page will let an agent do — and what it MUST confirm with the
visitor first — is declared in [agent-permissions.json](https://discord.demo.busymate.ai/agent-permissions.json).
